			...
			try {
				...
				try {
					...
					try {
						...
						try {
							tempMarketOperator = marketOperatorBean
									.getMarketOperator(new MarketOperatorID(market, operator));

							// for(int i = 0; i < 30000; i++){
							dataBean.addFailureEvent(new FailureEvent(date, tempEventCause, tempFailureClass,
									tempUserEventType, tempMarketOperator, cellID, duration, neVersion, imsi));
