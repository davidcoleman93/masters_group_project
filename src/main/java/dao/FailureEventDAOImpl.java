...
public class FailureEventDAOImpl implements FailureEventDAOLocal {
	...
	public Collection<?> getAllUniqueUETypes() {
		return em.createQuery("SELECT DISTINCT userEventType.tac FROM FailureEvent").getResultList();
	}

	public Collection<?> getAllUniqueIMSIs() {
		return em.createQuery("SELECT DISTINCT imsi FROM FailureEvent").getResultList();
	}

	public Collection<?> getAllUniquePhoneModels() {
		return em.createQuery("SELECT DISTINCT model FROM UserEventType ").getResultList();
	}
	...
	public Object specificFailurePerPeriod(Date startDate, Date endDate, Long imsi) {
		return em.createQuery("SELECT COUNT(o) FROM FailureEvent o WHERE o.imsi=:imsi AND o.dateTime BETWEEN ?1 AND ?2")
				.setParameter(1, startDate).setParameter(2, endDate).setParameter("imsi", imsi).getSingleResult();
	}
	...
	public Collection<?> getFailEventsUsingImsiGroupedByCauseCode(Long imsi) {
		return (List<Object[]>) em
				.createQuery(
						"SELECT fe.eventCause, count(fe) FROM FailureEvent fe WHERE fe.imsi=:imsi GROUP BY fe.eventCause.eventCauseID.causeCode")
				.setParameter("imsi", imsi).getResultList();
	}
	...
	public Object failureCountPerModel(Date startDate, Date endDate, String model) {
		return em
				.createQuery(
						"SELECT COUNT(o) FROM FailureEvent o WHERE o.userEventType.model=:model AND o.dateTime BETWEEN ?1 AND ?2")
				.setParameter("model", model).setParameter(1, startDate).setParameter(2, endDate).getSingleResult();
	}
	...
	public Collection<?> callDataPerPeriod(Date startDate, Date endDate) {
		return (List<Object[]>) em
				.createQuery(
						"SELECT DISTINCT o.imsi ,COUNT(o), SUM(o.duration) FROM FailureEvent o WHERE o.dateTime BETWEEN ?1 AND ?2 GROUP BY o.imsi")
				.setParameter(1, startDate).setParameter(2, endDate).getResultList();
	}
	...
	public Collection<?> getFailEventAndCauseCodeByUEType(String model) {
		return (List<Object[]>) em
				.createQuery(
						"SELECT fe.eventCause, count(fe) FROM FailureEvent fe WHERE fe.userEventType.model=:model GROUP BY fe.eventCause")
	...
	public Collection<?> topTenCallFailurePerPeriod(Date startDate, Date endDate) {
		return (List<Object[]>) em
				.createQuery(
						"SELECT o.marketOperator, o.cellID, count(o) as total FROM FailureEvent o WHERE o.dateTime BETWEEN ?1 AND ?2 GROUP BY o.marketOperator, o.cellID ORDER BY total DESC ")
				.setParameter(1, startDate).setParameter(2, endDate).setMaxResults(10).getResultList();
	}
	...
	public Collection<?> getTopTenIMSIsForFailureClassPerPeriod(Date startDate, Date endDate) {
		...
		return (List<Object[]>) em
				.createQuery(
						"SELECT o.imsi, COUNT(o) AS countIMSI FROM FailureEvent o WHERE o.dateTime BETWEEN ?1 AND ?2 GROUP BY o.imsi ORDER BY countIMSI DESC")
				.setParameter(1, startDate).setParameter(2, endDate).setMaxResults(10).getResultList();
	}
	...
	public Collection<?> getTopTenNodeFailuresPercentage() {
		...
		return (List<Object[]>) em
				.createQuery(
						"SELECT o.marketOperator, o.cellID, count(o) as numFailures, (count(o)/:totalNum)*100 from FailureEvent o group by o.marketOperator, o.cellID order by numFailures DESC")
				.setParameter("totalNum", totalNum).setMaxResults(10).getResultList();
	}

	// User Story #14
	public Collection<?> getIMSisForFailureClass(Integer failureClass) {
		return (List<FailureEvent>) em
				.createQuery("SELECT DISTINCT fe FROM FailureEvent fe WHERE fe.failureClass.failureClass=:failureClass")
				.setParameter("failureClass", failureClass).getResultList();
	}
