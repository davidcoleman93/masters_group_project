...
ex.push(".xls");
ex.push("xml");
if (process.env.FMTS === "full")
    process.env.FMTS = ex.join(":");
if (process.env.FMTS)
    ex = process.env.FMTS.split(":").map(function (x) {
        return x[0] === "." ? x : "." + x;
    });
var exp = ex.map(function (x) {
    return x + ".pending";
});
function test_file(x) {
    return ex.indexOf(x.substr(-5)) >= 0 || exp.indexOf(x.substr(-13)) >= 0 || ex.indexOf(x.substr(-4)) >= 0 || exp.indexOf(x.substr(-12)) >= 0;
}
    ...
    var getfile = function (dir, x, i, type) {
        ...
        if (x.substr(-5) === ".xlsb") {
            ...
            if (!fs.existsSync(name))
                name = (dir + root + '.xlsx.' + i + type);
            if (!fs.existsSync(name))
                name = (dir + root + '.xlsm.' + i + type);
            if (!fs.existsSync(name))
                name = (dir + root + '.xls.' + i + type);
        }
        if (x.substr(-4) === ".xls") {
            root = x.slice(0, -4);
            if (!fs.existsSync(name))
...
describe('parse options', function () {
    ...
    describe('cell', function () {
        it('XLSX should generate HTML by default', function () {
            ...
            Object.keys(ws).forEach(function (addr) {
                if (addr[0] === "!" || !ws.hasOwnProperty(addr))
                    return;
    ...
    describe('book', function () {
        ...
        var ckf = function (wb, fields, exists) {
...
describe('input formats', function () {
    ...
    it('should default to base64 type', function () {
        assert.throws(function () {
            X.read(fs.readFileSync(paths.cstxls, 'binary'));
        });
        assert.throws(function () {
            X.read(fs.readFileSync(paths.cstxml, 'binary'));
        });
        assert.throws(function () {
            X.read(fs.readFileSync(paths.cstxlsx, 'binary'));
        });
        assert.throws(function () {
            X.read(fs.readFileSync(paths.cstxlsb, 'binary'));
        });
...
function deepcmp(x, y, k, m, c) {
    ...
    if (s < 0)
        return assert[c < 0 ? 'notEqual' : 'equal'](x[k], y[k], m);
...
function diffsty(ws, r1, r2) {
    ...
    stykeys.forEach(function (m) {
        var c = -1;
        if (styexc.indexOf(r1 + "|" + r2 + "|" + m) > -1)
            c = 1;
        else if (styexc.indexOf(r2 + "|" + r1 + "|" + m) > -1)
            c = 1;
...
describe('parse features', function () {
    ...
    describe('merge cells', function () {
        ...
        it('should have !merges', function () {
            ...
            var m = [wb1, wb2, wb3, wb4, wb5].map(function (x) {
    ...
    describe('should correctly handle styles', function () {
        ...
        before(function () {
            ...
            rn2 = function (r) {
                return [].concat.apply([], r.split(",").map(rn));
            };
        });
        var ranges = [
        ...
        it('correct styles', function () {
            var stylesxls = ranges.map(function (r) {
                return rn2(r)[0];
            }).map(function (r) {
                return wsxls[r].s;
            });
            var stylesxlsx = ranges.map(function (r) {
                return rn2(r)[0];
            }).map(function (r) {
                return wsxlsx[r].s;
            });
    ...
    describe('should preserve dates', function () {
        seq(16).forEach(function (n) {
            ...
            it('[' + a + '] -> (' + b + ') -> [' + c + '] -> (' + d + ')', function () {
                ...
                var m = [wb1, wb2].map(function (x) {
                    return x.Sheets[sheet][addr];
                });
                assert.equal(m[0].w, m[1].w);

                /* wb1 cellDates */
                if (dh && di)
                    assert.equal(m[0].t, 'd');
                //else if(a !== 'd' && di) assert.equal(m[0].t, 'd'); /* TODO */
                else
                    assert.equal(m[0].t, 'n');
                /* wb2 cellDates */
                if (dh && di && dj && dk)
                    ...
                    assert.equal(m[1].t, 'n');

                if (m[0].t === m[1].t)
                    assert.equal(m[0].v, m[1].v);
                else if (m[0].t === 'd')
                    assert(Math.abs(datenum(new Date(m[0].v)) - m[1].v) < 0.01); /* TODO: 1sec adjustment */
            });
        });
    });
    ...
    function seeker(json, keys, val) {
        for (var i = 0; i != json.length; ++i) {
            for (var j = 0; j != keys.length; ++j) {
                if (json[i][keys[j]] === val)
                    throw new Error("found " + val + " in row " + i + " key " + keys[j]);
            }
        }
    }
    var data, ws;
